...
const FROM = `${process.env.FROM_NAME || 'E-Prescription System'} <${process.env.FROM_EMAIL!}>`
  ...
  await send(to, 'Активация аккаунта — E-Prescription', `
    <div style="font-family:Arial,sans-serif;max-width:560px;margin:0 auto">
      <div style="background:#1D9E75;padding:24px 32px;border-radius:8px 8px 0 0">
        ...
        <p style="color:#d1fae5;margin:4px 0 0;font-size:13px">Казахстан</p>
      </div>
      <div style="background:#fff;padding:32px;border:1px solid #e5e7eb;border-top:none;border-radius:0 0 8px 8px">
        ...
        <p style="color:#6b7280;font-size:12px">Ссылка действительна 24 часа. Если вы не ожидали это письмо — проигнорируйте его.</p>
  ...
  await send(to, 'Сброс пароля — E-Prescription', `
    <div style="font-family:Arial,sans-serif;max-width:560px;margin:0 auto">
      <div style="background:#2E5E9B;padding:24px 32px;border-radius:8px 8px 0 0">
        <h2 style="color:#fff;margin:0;font-size:20px">E-Prescription System</h2>
        <p style="color:#bfdbfe;margin:4px 0 0;font-size:13px">Казахстан</p>
      </div>
      <div style="background:#fff;padding:32px;border:1px solid #e5e7eb;border-top:none;border-radius:0 0 8px 8px">
        <p style="font-size:15px;color:#111">Здравствуйте, <b>${fullName}</b>!</p>
        <p style="color:#374151;font-size:14px;line-height:1.6">
  ...
  await send(to, `Код подтверждения — E-Prescription`, `
    <div style="font-family:Arial,sans-serif;max-width:560px;margin:0 auto">
      ...
      <div style="background:#fff;padding:32px;border:1px solid #e5e7eb;border-top:none;border-radius:0 0 8px 8px">
        <p style="font-size:15px;color:#111">Здравствуйте, <b>${fullName}</b>!</p>
        <p style="color:#374151;font-size:14px;line-height:1.6">
          Вы запросили изменение ${fieldRu}.<br>
          Введите код подтверждения в приложении:
        </p>
        <div style="text-align:center;margin:32px 0">
        ...
        <p style="color:#6b7280;font-size:12px;text-align:center">Код действителен 10 минут. Не сообщайте его никому.</p>
      </div>
    </div>
  `)
}

// ── 4. Prescription QR link to patient ────────────────────────────
export async function sendPrescriptionEmail(
  ...
  const medsHtml = medications.map(m =>
    `<tr>
      ...
      <td style="padding:8px 12px;border-bottom:1px solid #f3f4f6;font-size:14px;color:#374151;text-align:center">${m.dosage}</td>
      <td style="padding:8px 12px;border-bottom:1px solid #f3f4f6;font-size:14px;color:#374151;text-align:center">${m.qtyPrescribed} шт.</td>
    </tr>`
  ).join('')
  ...
  await send(to, 'Электронный рецепт — E-Prescription', `
    <div style="font-family:Arial,sans-serif;max-width:560px;margin:0 auto">
      ...
      <div style="background:#fff;padding:32px;border:1px solid #e5e7eb;border-top:none;border-radius:0 0 8px 8px">
        ...
        <table style="width:100%;border-collapse:collapse;margin:24px 0;border:1px solid #e5e7eb;border-radius:8px;overflow:hidden">
          <thead>
            <tr style="background:#f9fafb">
              <th style="padding:10px 12px;text-align:left;font-size:13px;color:#6b7280;font-weight:600;border-bottom:1px solid #e5e7eb">Препарат</th>
              <th style="padding:10px 12px;text-align:center;font-size:13px;color:#6b7280;font-weight:600;border-bottom:1px solid #e5e7eb">Дозировка</th>
              <th style="padding:10px 12px;text-align:center;font-size:13px;color:#6b7280;font-weight:600;border-bottom:1px solid #e5e7eb">Количество</th>
            </tr>
          </thead>
        ...
        <div style="background:#fffbeb;border:1px solid #fcd34d;border-radius:8px;padding:16px;margin-top:8px">
          <p style="margin:0;font-size:13px;color:#92400e">
            <b>Важно:</b> Рецепт действителен 30 дней. При посещении аптеки предъявите этот QR-код и удостоверение личности.
          </p>
        </div>

        <p style="color:#6b7280;font-size:12px;margin-top:24px">Если вы не ожидали это письмо — обратитесь в вашу клинику.</p>
      </div>
    </div>
  `)
}
